...
class _ReviewEquipmentEntryScreenState
  ...
  String _transcript =
      'Hey SiteTrack, log an equipment entry for the North District site. '
  ...
  List<ExtractedField> get _extractedFields => [
    ExtractedField(
      icon: Icons.precision_manufacturing_outlined, label: 'Equipment',
      value: _nameCtrl.text, isHighlight: true, confidence: 0.97,
    ),
    ExtractedField(
      icon: Icons.category_outlined, label: 'Type',
      value: _typeCtrl.text, isEmpty: _typeCtrl.text.isEmpty, confidence: 0.88,
    ),
    ExtractedField(
      icon: Icons.person_outlined, label: 'Operator',
      value: _operatorCtrl.text, isEmpty: _operatorCtrl.text.isEmpty, confidence: 0.85,
    ),
    ExtractedField(
      icon: Icons.av_timer_outlined, label: 'Usage Hours',
      value: '${_hoursCtrl.text} hrs', confidence: 0.94,
    ),
    ExtractedField(
      icon: Icons.local_gas_station_outlined, label: 'Fuel Used',
      value: '${_fuelCtrl.text} L', isEmpty: _fuelCtrl.text.isEmpty, confidence: 0.82,
    ),
    ExtractedField(
      icon: Icons.attach_money_outlined, label: 'Rate / Hour',
      value: '₹ ${_rateCtrl.text}', confidence: 0.93,
    ),
    ExtractedField(
      icon: Icons.layers_outlined, label: 'Floor / Zone',
      value: _selectedFloor ?? '', isEmpty: _selectedFloor == null, confidence: 0.72,
    ),
    ExtractedField(
  ...
  Future<void> _confirm(BuildContext ctx) async {
    ...
    ctx.read<ProjectProvider>().addEntry(
      EntryModel(
        ...
        type: EntryType.equipment, amount: double.tryParse(_hoursCtrl.text) ?? 0.0,
    ...
    Navigator.pushNamed(
      ctx, '/logs',
      arguments: {
        'type': 'equipment', 'name': _nameCtrl.text,
        'newEntry': em.Entry(
          id: entryId, type: em.EntryType.equipment,
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    VoiceStatusHeader(
                      state: _voiceState, entryTypeLabel: 'Equipment',
                      confidence: 94.2, onMicTap: _handleMicTap,
                    ...
                    if (isParsed) ...[
                      ...
                      EntrySectionCard(
                        child: Column(
                          ...
                          children: [
                            ...
                            EntryUnderlineField(controller: _nameCtrl, hint: 'Equipment name'),
                            const SizedBox(height: 18),
                            const EntryFieldLabel('Equipment Type'),
                            const SizedBox(height: 8),
                            EntryUnderlineField(controller: _typeCtrl, hint: 'e.g. Heavy, Light'),
                            const SizedBox(height: 18),
                            const EntryFieldLabel('Operator Name'),
                            const SizedBox(height: 8),
                            EntryUnderlineField(controller: _operatorCtrl, hint: 'Operator name'),
                            const SizedBox(height: 18),
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                            ...
                            const SizedBox(height: 8),
                            EntryUnderlineField(
                              controller: _fuelCtrl, hint: '0', suffix: 'L',
                              keyboardType: TextInputType.number,
                            ),
                            const SizedBox(height: 18),
                      ...
                      CostSummaryCard(
                        totalAmount: _computedTotal, label: 'Equipment Usage Cost',
                        subtotals: [
