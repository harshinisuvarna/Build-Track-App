class InventoryItem {
  ...
  factory InventoryItem.fromJson(Map<String, dynamic> json) {
    final String rawCat = (json['category'] ?? '').toString().trim().toLowerCase();
    ...
    return InventoryItem(
      id: json['_id'] ?? '',
      name: json['materialName'] ?? json['itemName'] ?? json['title'] ??
            json['name'] ?? json['brand'] ?? 'Unknown',
      totalPurchased: (json['purchased'] ?? json['totalPurchased'] ?? json['quantity'] ?? 0).toDouble(),
      totalUsed: (json['used'] ?? json['totalUsed'] ?? 0).toDouble(),
      closingStock: (json['closingStock'] ?? json['quantity'] ?? json['purchased'] ?? 0).toDouble(),
