...
class _TransactionLogsScreenState extends State<TransactionLogsScreen> {
  ...
  int get _totalAdded {
    return _allLogs
        .where((l) => l['isPositive'] == true)
        .fold(0, (sum, l) {
          final v = int.tryParse(
                  l['amount'].toString().replaceAll('+', '').trim()) ??
              0;
          return sum + v;
        });
  }
  ...
  String get _unitLabel {
    switch (_itemType) {
      case 'labour':    return 'workers';
      case 'equipment': return 'hrs';
  ...
  // FIX: title column wrapped in Expanded so filter chips never overflow
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            const Text('Entries you add will appear here',
  ...
  Widget _logItem(BuildContext context, Map<String, dynamic> log) {
    ...
    // ✅ FIX: Material + InkWell for proper ripple + interaction safety
    return Material(
      color: Colors.white,
              ...
              Column(
                ...
                children: [
                  ...
                  Text(
                    log['date'] as String? ?? '',
                    style: const TextStyle(
                        color: textGray, fontSize: 10.5),
                  ),
                  if (receipt != null && receipt.isNotEmpty)
                    const Padding(
                      padding: EdgeInsets.only(top: 2),
                      child: Icon(Icons.attach_file,
                          color: textGray, size: 12),
                    ),
                ],
              ),
            ],
